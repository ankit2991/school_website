...
import {
    ...
    getVehicleProviderDelete,
    getVehicleProviderDetail,
    getVehicleProviderInsertUpdate
} from '../../services/api'
...
function Vehicle_Provider() {
    ...
    const handleEdit = async (id) => {
        try {
            setSearched(true)
            const res = await getVehicleProviderDetail(id, instId, sessId)
            if (res?.Table?.length) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            setSearched(true)
            const res = await getVehicleProviderDelete(id)
            const msg = res?.Table?.[0]?.Column1 || ""
            if (msg.startsWith("M103")) {
                alert("Vehicle Provider Deleted")
                fetchProviders()
