...
function Hostel_Fee2() {
   ...
   const handleDeletePaidFee = async (row) => {
      ...
      try {
         ...
         // 3. Call the API
         const res = await deleteHostelReceipt(rcID);
   ...
   const refreshStudentFees = async () => {
      try {
         ...
         const res = await getHostelFeesDetails(
            instId,
            sessionId,
            classId,
            studentId
   ...
   useEffect(() => {
      ...
      async function fetchHostelFees() {
         try {
            setLoading(true);
            const res = await getHostelFeesDetails(
               instId,
            ...
            // Inside fetchHostelFees useEffect
   ...
   const handleSave = async () => {
      ...
      try {
         let res = await hostelFeesInsert(payload);
         ...
         if (code === "M200") {
            ...
            const retryRes = await hostelFeesInsert(payload);
